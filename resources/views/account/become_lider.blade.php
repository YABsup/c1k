...
<div class="col Fill" style="width: auto;">
  <div class="col-md-12 change-text-title-person ">
    <h4>@lang('anketa.h4_lider')</h4>
    <div class="table-info-person-text">@lang('anketa.welcome_lider')</div>
  </div>
  ...
  <form method="POST" action="{{ route('account.anketa') }}" class="form">
    ...
    <input type="hidden" name="type" value="lider">
    @foreach($form_fields as $field)
    <div class="text-person_data">
      <label for="inputFor{{ $field }}">@lang('anketa.form_'.$field)</label>
      <input class="form-control" type="text" style="color: #000000" class="" name="{{ $field }}" id="input{{ $field }}"  placeholder="@lang('anketa.placeholder_'.$field)"  required>
    </div>
    @endforeach
